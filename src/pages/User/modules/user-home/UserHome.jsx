...
import { getUserDetail, getUserPlayList, getUserRecord } from '../../../../services/apis'
      ...
      {userInfo ? <div className='user-info'>
        ...
        <div className='info-content'>
          <div className='name-wrap'>
            ...
            <span
              className={userInfo.profile.gender === 1 ? 'level blue' : 'level pink'}>LV. {userInfo.level} {userInfo.profile.gender === 1 ?
              <IconMale/> : <IconFemale/>}</span>
      ...
      {/* 用户的播放记录，当前用户才可以访问 */}
      {
        recordList && recordList.length ? <>
          ...
          <ul className='record-list'>
            {recordList ? recordList.slice(0, 10).map((r, i) => <RecordItem key={r.song.id} record={r} index={i + 1}
                                                                            showNum={user.userId === uid}></RecordItem>) : null}
          </ul>
