        ...
        <div className='simi-content'>
          {simiList ? simiList.map(l => <div key={l.id} className='simi-item'>
            ...
            <div className='desc'>
              ...
              <span className='text'>by <Link to={'/user/home?uid=' + l.creator.userId}
                                              className='name'>{l.creator.nickname}</Link></span>
            </div>
          </div>) : null}
        </div>
      </div>
          ...
          {simiSongs ? simiSongs.map(s => <div className='simi-item song-item' key={s.id}>
            <div className='content'>
              ...
              <div className='artists'>
                {s.artists.map((a, i) => (i === 0 ?
                  <Link className='item' key={a.id} to={'/find/artist?id=' + a.id}>{a.name}</Link> :
                  <span className='divider' key={a.id}>/<Link className='item'
