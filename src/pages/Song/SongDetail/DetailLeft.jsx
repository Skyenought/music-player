...
import { getLyric, getMusicPlayUrl, getSongComment, getSongUrl } from '../../../services/apis'
import { Button, Pagination, Space, Spin, Toast } from '@douyinfe/semi-ui'
import {
  IconChevronDown,
  IconChevronUp,
  IconComment,
      ...
      <div className='top-content'>
        ...
        <div className='list-content'>
          ...
          <div className='row'>
            ...
            <div className='wrap'>
              {detail.ar.map((a, i) => (i === 0 ?
                  <Link key={a.id} className='item' to={'/find/artist?id=' + a.id}>{a.name}</Link> :
                  <span key={a.id} className='divider'>/<Link className='item'
                                                              to={'/find/artist?id=' + a.id}>{a.name}</Link></span>
              ))}
            </div>
          </div>
          <div className='row'>
