  ...
  return (
    <div className={style['top-left-nav']}>
      <dl className='list'>
        ...
        {firstNav.length ? firstNav.map(n => <Link to={`/find/toplist?id=${n.id}&text=${n.updateFrequency}`} key={n.id}>
          <dd className={n.id == id ? 'row active' : 'row'}>
            <LazyLoad>
      ...
      <dl className='list mt20'>
        <dt className='title'>云音乐特色榜</dt>
        {secondNav.length ? secondNav.map(n => <Link to={`/find/toplist?id=${n.id}&text=${n.updateFrequency}`}
                                                     key={n.id}>
          <dd className={n.id == id ? 'row active' : 'row'}>
            <img src={n.coverImgUrl} alt=''/>
            <div className='content'>
