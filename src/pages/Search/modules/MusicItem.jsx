...
import { checkMusic, getMusicPlayUrl } from '../../../services/apis'
  ...
  const items = songs.map((song, index) => (
    <li key={song.id} className={`${utils.isEven(index + 1) ? 'search-item' : 'search-item even'}`}>
      ...
      <div className='singers'>
        {song.artists && song.artists.map((item, i) => <Link className='singer-name' to={'/find/artist?id=' + item.id}
                                                             key={item.id}>{i === 0 ? item.name : `/${item.name}`}</Link>)}
      </div>
